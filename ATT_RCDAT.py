...
class Attenuator:
    ...
    def set(self, command):
        if self.pwd_enabled:
            ...
            pwd = ''
        s = '''http://{}:{}/PWD={}{}'''.format(self.ip, self.port, pwd, command)
        #print(s)
        url = req.urlopen(s)
        return  url.read().decode('utf-8')
    ...
    def setup(self, mode='N', ip=None, mask=None, gateway=None, port=None, usb_adr=None, password=None, on_att=30):
        ...
        if ip == None:
            ...
            if currentIp != ip:
                print("changing ip from {} to {}".format(currentIp,ip))
                if not self.set_ip(ip):
                    print("setting ip:{} failed".format(ip))
            else:
    ...
    def att_ramp(self, start=None, stop=None, step=None, time_ms=None ):
        ...
        for att in atts:
            self.set_att(att)
            if att != self.get_att():
                print("Error set attenuation step {}".format(att))
    ...
    def get_ip(self):
        ...
        return r
    def set_ip(self, ip):
        r = int(self.set(':ETHERNET:CONFIG:IP:{}'.format(ip)))
        return r
    ##--------------------------------------------------------------------------
    def set_mask(self, mask):
        r = self.set(':ETHERNET:CONFIG:SM:{}'.format(mask))
        return int(r)
    def get_mask(self):
    ...
    def set_gate(self, gateway):
        r = self.set(':ETHERNET:CONFIG:NG:{}'.format(gateway))
        return int(r)
    def get_gate(self):
    ...
    def set_portHTTP(self, port):
        r = int(self.set(':ETHERNET:CONFIG:HTPORT:{}'.format(port)))
        return r
    ##--------------------------------------------------------------------------
    def set_enablePSSWD(self, flag):
        r = int(self.set(':ETHERNET:CONFIG:PWDENABLED:{}'.format(flag)))
        return r
    ...
    def set_psswdHTTP(self, password):
        r = self.set(':ETHERNET:CONFIG:PWD:{}'.format(password))
        return int(r)
    ...
    def set_DHCP(self, enable):
        r = self.set(':ETHERNET:CONFIG:DHCPENABLED:{}'.format(enable))
        return int(r)
    def get_DHCP(self):
    ...
    def set_att(self, value):
        ...
        r = int(self.set(':SETATT={}'.format(value)))
        if r == 0:
            ...
            print('Requested attenuation was higher than the allowed range,\
                the attenuation was set to the device’s maximum allowed value')
        return True
    ...
    def set_startUp_att(self, value):
        r = int(self.set(':STARTUPATT:VALUE:{}'.format(value)))
    ...
    def set_startUp_mode(self, value):# L=Last Value, F=Fixed Value, N=Default
        r = int(self.set(':STARTUPATT:INDICATOR:{}'.format(value)))
        return r # 0 failed, 1 successful
    ...
    def set_pointsHop(self, points):
        r = self.set(':HOP:POINTS:{}'.format(points))
        return int(r)
    ...
    def set_directionHop(self, direction):
        r = self.set(':HOP:DIRECTION:{}'.format(direction))
        return int(r)
    ...
    def set_indexPointHop(self, index):
        r = self.set(':HOP:POINT:{}'.format(index))
        return int(r)
    ...
    def set_dwellTimeUnitsHop(self, units):
        r = self.set(':HOP:DWELL_UNIT:{}'.format(units))
        return int(r)
    ...
    def set_dwellTimeHop(self, time):
        r = self.set(':HOP:DWELL:{}'.format(time))
        return int(r)
    ...
    def set_attPointHop(self, point_att):
        r = self.set(':HOP:ATT:{}'.format(point_att))
        return int(r)
    ...
    def set_directionSweep(self, direction):
        r = self.set(':SWEEP:DIRECTION:{}'.format(direction))
        return int(r)
    ...
    def set_startAttSweep(self, start):
        r = self.set(':SWEEP:START:{}'.format(start))
        return int(r)
    ...
    def set_stopAttSweep(self, stop):
        r = self.set(':SWEEP:STOP:{}'.format(stop))
        return int(r)
    ...
    def set_stepAttSweep(self, stop):
        r = self.set(':SWEEP:STEPSIZE:{}'.format(stop))
        return int(r)
    ...
    def enableSweepMode(self, enable):
        a = 'ON' if enable else 'OFF'
        r = self.set(':SWEEP:MODE:{}'.format(a))
        return int(r)
